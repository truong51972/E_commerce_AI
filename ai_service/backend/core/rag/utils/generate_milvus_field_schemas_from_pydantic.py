...
def generate_milvus_field_schemas_from_pydantic(
    ...
    for field_name, model_field in pydantic_model.model_fields.items():
        ...
        if model_field.json_schema_extra and model_field.json_schema_extra.get('exclude', False):
            continue
        ...
        if not dtype:
            raise ValueError(f"Missing 'dtype' in milvus_config for field '{field_name}'.")
        ...
        if dtype == DataType.VARCHAR:
            ...
            if not milvus_field_params["element_type"]:
                raise ValueError(f"Missing 'element_type' for ARRAY field '{field_name}'.")
            ...
            if not isinstance(embedding_dim, int) or embedding_dim <= 0:
                raise ValueError(f"Invalid 'embedding_dim' for FLOAT_VECTOR field '{field_name}'. Must be a positive integer.")
        ...
        for key, value in milvus_config.items():
            if key not in milvus_field_params and key not in ["dtype", "element_type", "max_length", "max_capacity", "dim", "is_primary", "auto_id"]:
...
if __name__ == "__main__":
    ...
    for field in milvus_fields:
        print(f"  - Name: {field.name}, Dtype: {field.dtype}, Params: {field.params}, Primary: {field.is_primary}")